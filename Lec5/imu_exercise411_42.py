...
for line in f:
	...
	## Calculate pitch angle according to eq. 28 from document Tilt Sensing
	#myValue = math.atan2(acc_y, math.sqrt(math.pow(acc_x,2) + math.pow(acc_z,2)) )
	
	## Integration of gyro measurements
	dt = ts_now - ts_prev  # compute time difference between measurements
	myValue = myValue + dt*gyro_z # integrate measurements
